...
impl FileSystemRepository for StandardFileSystem {
    ...
    async fn read_file(&self, path: &Path) -> RepositoryResult<Vec<u8>> {
        ...
        let mut file = File::open(&absolute_path).map_err(|e| Self::io_to_domain_error(e, path))?;

        let mut contents = Vec::new();
        file.read_to_end(&mut contents).map_err(|e| {
            DomainError::InternalError(format!("Failed to read file {}: {}", path.display(), e))
        })?;
    ...
    async fn write_file(&self, path: &Path, content: &[u8]) -> RepositoryResult<()> {
        ...
        self.ensure_parent_dir(&absolute_path).map_err(|e| {
        ...
        let mut file = OpenOptions::new()
            ...
            .open(&absolute_path)
            .map_err(|e| {
                DomainError::InternalError(format!("Failed to open file for writing: {}", e))
            })?;
    ...
    async fn delete_file(&self, path: &Path) -> RepositoryResult<()> {
        ...
        fs::remove_file(&absolute_path).map_err(|e| Self::io_to_domain_error(e, path))?;
    ...
    async fn rename_file(&self, from: &Path, to: &Path) -> RepositoryResult<()> {
        ...
        self.ensure_parent_dir(&absolute_to).map_err(|e| {
            DomainError::InternalError(format!("Failed to create parent directory: {}", e))
        })?;
    ...
    async fn remove_directory(&self, path: &Path) -> RepositoryResult<()> {
        ...
        fs::remove_dir_all(&absolute_path).map_err(|e| Self::io_to_domain_error(e, path))?;
...
mod tests {
    ...
    async fn test_file_operations() {
        ...
        fs_repo
            .rename_file(test_path, renamed_path)
            .await
        ...
        fs_repo
            .write_file(file_in_dir_path, b"File in directory")
            .await
            .expect("Failed to write file");

        // List directory
        let dir_contents = fs_repo
