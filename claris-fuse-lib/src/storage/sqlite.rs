...
impl SqliteStorage {
    ...
    fn time_to_sqlite(time: SystemTime) -> i64 {
        time.duration_since(UNIX_EPOCH).unwrap_or(Duration::from_secs(0)).as_secs() as i64
    }
...
impl StorageBackend for SqliteStorage {
    async fn init(&self) -> StorageResult<()> {
        ...
        conn.execute(
            &format!("PRAGMA synchronous = {}", if self.synchronous { "NORMAL" } else { "OFF" }),
            [],
        ).map_err(|e| StorageError::DatabaseError(format!("Failed to set synchronous mode: {}", e)))?;
        
        // Initialize schema
        SqliteSchema::initialize(&conn)
            .map_err(|e| StorageError::DatabaseError(format!("Failed to initialize schema: {}", e)))?;
...
impl VersionStorage for SqliteStorage {
    async fn record_version(
        ...
        debug!("Recorded new version {} for path {}", version_id, path.display());
        Ok(version_id)
    }
    ...
    async fn get_version_content(&self, version_id: i64) -> StorageResult<Option<Vec<u8>>> {
        ...
        let mut stmt = conn.prepare("SELECT id FROM versions WHERE id = ?")
            .map_err(|e| StorageError::DatabaseError(format!("Query error: {}", e)))?;
            
        let version_exists = stmt.query_row(params![version_id], |_| Ok(()))
            .is_ok();
        ...
        // Get content
        let mut stmt = conn.prepare("SELECT data FROM contents WHERE version_id = ?")
            .map_err(|e| StorageError::DatabaseError(format!("Query error: {}", e)))?;
        ...
        match result {
            ...
            Err(e) => Err(StorageError::DatabaseError(format!("Content query error: {}", e))),
        }
    }
    ...
    async fn get_versions(
        ...
        let conn_guard = self.connection.lock().await;
        let conn = conn_guard.as_ref().ok_or_else(|| 
            StorageError::StorageError("Database not initialized".to_string())
        )?;
