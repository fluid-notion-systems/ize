...
mod model_tests {
    ...
    fn setup_test_db() -> SqliteConnection {
        ...
        // Create the tables
        sql_query("CREATE TABLE file_paths (
            id INTEGER PRIMARY KEY,
            path TEXT NOT NULL,
            created_at INTEGER NOT NULL,
            last_modified INTEGER NOT NULL
        )").execute(&conn).expect("Failed to create file_paths table");
        
        sql_query("CREATE TABLE versions (
            id INTEGER PRIMARY KEY,
        ...
        )").execute(&conn).expect("Failed to create versions table");
        
        sql_query("CREATE TABLE contents (
            version_id INTEGER PRIMARY KEY,
            data BLOB NOT NULL,
            FOREIGN KEY (version_id) REFERENCES versions(id)
        )").execute(&conn).expect("Failed to create contents table");
