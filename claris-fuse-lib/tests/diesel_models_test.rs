...
mod common;
use common::{setup_test_db, cleanup, get_unix_timestamp};
...
fn test_create_and_retrieve_version() {
    ...
    assert_eq!(retrieved_version.description, Some("Test version".to_string()));
...
fn test_create_and_search_fts() {
    ...
    // Query the FTS table
    let results = diesel::sql_query("SELECT * FROM versions_fts WHERE description MATCH 'searchable unique'")
        .load::<DbVersionFts>(&mut conn)
