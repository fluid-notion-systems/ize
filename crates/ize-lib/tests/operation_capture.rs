...
fn test_passthrough2_basic_setup() -> std::io::Result<()> {
    ...
    let _fs = PassthroughFS::new(source_dir.path(), mount_dir.path())?;
    ...
    fs::write(source_dir.path().join("test.txt"), "content")?;
    fs::create_dir(source_dir.path().join("testdir"))?;

    // Verify files exist
    assert!(source_dir.path().join("test.txt").exists());
    assert!(source_dir.path().join("testdir").exists());
