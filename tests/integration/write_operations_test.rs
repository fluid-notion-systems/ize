...
impl FilesystemMountHarness {
    ...
    fn new() -> io::Result<Self> {
        ...
        fs::write(&db_path, b"CLARIS_DB_V1")?;
...
fn test_simple_file_write_creates_dirty_entry() {
    ...
    harness.test_write_operations(|ctx| {
        // Write a simple file
        ctx.write_file("test.txt", b"Hello, Claris!").unwrap();

        // Verify it exists in both mount and source
        ctx.verify_in_mount("test.txt", b"Hello, Claris!").unwrap();
        ctx.verify_in_source("test.txt", b"Hello, Claris!").unwrap();
    });
